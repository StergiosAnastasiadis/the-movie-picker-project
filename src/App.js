  ...
  return (
    <div className="App">
      ...
      <div className="add_your_flex_styling_check_wrap">
        {data.map((item) => (<Card key={item.id} id={item.id} title={item.title} poster_path={item.poster_path} moviesInsideCart={moviesInsideCart} setMoviesInsideCart={setMoviesInsideCart} />))}
      </div>

      <Cart setMoviesInsideCart={setMoviesInsideCart} />
    </div>
  );
}
